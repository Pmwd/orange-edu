...
import com.orange.edu.content.feignclient.SearchServiceClient;
import com.orange.edu.content.feignclient.po.CourseIndex;
import com.orange.edu.content.mapper.CourseBaseMapper;
import com.orange.edu.content.mapper.CourseMarketMapper;
import com.orange.edu.content.mapper.CoursePublishMapper;
import com.orange.edu.content.mapper.CoursePublishPreMapper;
...
import com.orange.edu.content.model.po.CourseMarket;
import com.orange.edu.content.model.po.CoursePublish;
import com.orange.edu.content.model.po.CoursePublishPre;
import com.orange.edu.content.service.CourseBaseInfoService;
import com.orange.edu.content.service.CoursePublishService;
...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if("202003".equals(auditStatus)){
            OrangeEduException.cast("当前为等待审核状态，审核完成可以再次提交。");
        }
        //本机构只允许提交本机构的课程
        if(!courseBase.getCompanyId().equals(companyId)){
            OrangeEduException.cast("不允许提交其它机构的课程。");
        }

        //课程图片是否填写
        if(StringUtils.isEmpty(courseBase.getPic())){
            OrangeEduException.cast("提交失败，请上传课程图片");
        }
        ...
        if(teachplanTree.size()<=0){
            OrangeEduException.cast("提交失败，还没有添加课程计划");
        }
        //转json
    ...
    public void publish(Long companyId, Long courseId) {
        ...
        if(coursePublishPre == null){
            OrangeEduException.cast("请先提交课程审核，审核通过才可以发布");
        }
        //本机构只允许提交本机构的课程
        ...
        if(!"202004".equals(auditStatus)){
            OrangeEduException.cast("操作失败，课程审核通过方可发布。");
        }
...
//            System.out.println(content);
            ...
            OrangeEduException.cast("课程静态化异常");
        }
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        ...
        if(course==null){
            OrangeEduException.cast("上传静态文件异常");
        }
    ...
    public Boolean saveCourseIndex(Long courseId) {
        ...
        if(!add){
            OrangeEduException.cast("添加索引失败");
        }
        return add;
    ...
    private void saveCoursePublish(Long courseId){
        ...
        CoursePublishPre coursePublishPre = coursePublishPreMapper.selectById(courseId);
        if(coursePublishPre == null){
            OrangeEduException.cast("课程预发布数据为空");
        }
